...
void setup() {
  ...
  Serial.println("System bezi (Blocking architektura). Cekam na pripojeni...");
}

void loop() {
  // Aktualizace systémů senzorů a Bluetooth vrstvy
  BLE.poll();
  BHY2.update();
  ...
  if (BLE.connected()) {
    ...
    // V případě ztráty spojení nebo jeho odmítnutí čekáme 100ms
    delay(100);
  }
}
